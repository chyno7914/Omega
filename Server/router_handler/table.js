...
exports.adminFloorSearch = (req, res, next) => {
    ...
    const sql = `SELECT t1.floor,max_room,IFNULL(use_room,0) use_room,decipher,tag_type 
                ...
                AND tid = (
                    SELECT tid FROM omega_tower WHERE tname = "${targetFlat}"
                )
                GROUP BY FLOOR
...
exports.pushFloor = (req, res, next) => {
    ...
    db.query(sql, (err, results) => {
        ...
        db.query(sql, (err, results) => {
            ...
            db.query(
                sql, { tid, floor: awaitAddFloor, max_room: width },
                (err, results) => {
                    if (err) return next(err);
                    res.cc("插入成功", 0);
                }
...
exports.editRoom = (req, res, next) => {
    ...
    const sql = `UPDATE omega_room 
                SET tol_bed = ${tolBed}, status = 
                CASE
                    WHEN ${tolBed} > use_bed THEN 300
                    WHEN ${tolBed} = use_bed THEN 301
                    ELSE status
                END
                WHERE tid = ${tid} AND rid = ${rid} AND ${tolBed} >= use_bed;`;
    db.query(sql, (err, results) => {
        if (err) return next(err);
...
exports.userLink = (req, res, next) => {
    const { target } = req.query
    const sql = `SELECT uid,username,user_pic,telephone FROM omega_users
                WHERE uid = (
                    SELECT uid FROM omega_students
                    WHERE sid = ${target}
                )`
    db.query(sql, (err, result) => {
        if (err) return next(err);
